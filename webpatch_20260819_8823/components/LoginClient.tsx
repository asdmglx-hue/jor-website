...
export default function LoginClient() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Ping last_seen_at on login
      if (proposal.id && !proposal.id.startsWith('admin:')) {
        supabase.rpc('update_last_seen', { p_proposal_id: proposal.id, p_source: 'web' }).then(() => {});
      }
  ...
  const handleForgotWhatsApp = async () => {
    ...
    const photoUrl = await uploadForgotPasswordCnicPhoto(forgotPhoto, digits);
    setForgotUploading(false);
    if (!photoUrl) { setForgotError('Failed to upload CNIC photo. Please check your connection and try again.'); return; }
        ...
        <div onClick={() => { if (!forgotUploading) { setShowForgotModal(false); setForgotCnic(''); setForgotPhoto(null); setForgotError(''); } }} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 380, padding: 24 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
              ...
              <span style={{ fontSize: 17, fontWeight: 800, color: '#1A1830' }}>Forgot Password</span>
            </div>

            {/* Description */}
            <div style={{ fontSize: 13, color: '#6B6893', marginBottom: 18, lineHeight: 1.5 }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '12px 14px', borderRadius: 12, border: `1.5px solid ${forgotError && !forgotCnic ? '#DC2626' : '#E8E6F5'}`, fontSize: 14, outline: 'none', color: '#1A1830', letterSpacing: 1, boxSizing: 'border-box' }}
            />
            ...
            <label style={{
              ...
              background: '#F7F6FF', border: `1.5px solid ${forgotError && !forgotPhoto ? '#DC2626' : '#E8E6F5'}`,
            }}>
              <input type="file" accept="image/*" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) { setForgotPhoto(f); setForgotError(''); } }} />
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={forgotPhoto ? '#29B286' : '#534AB7'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
              <span style={{ fontSize: 13, fontWeight: 600, color: forgotPhoto ? '#29B286' : '#534AB7', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 260 }}>
                {forgotPhoto ? forgotPhoto.name : 'Upload CNIC front photo'}
              </span>
            </label>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
              ...
              <button onClick={handleForgotWhatsApp} disabled={forgotUploading}
                style={{ flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '13px', borderRadius: 12, border: 'none', background: forgotUploading ? '#C4C2D8' : '#534AB7', color: '#fff', fontWeight: 800, fontSize: 14, cursor: forgotUploading ? 'not-allowed' : 'pointer' }}>
                {forgotUploading
