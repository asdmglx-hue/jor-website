        ...
        {steps.map((s, i) => {
          ...
          return (
            <div key={i} style={{ flex: 1, textAlign: 'center', cursor: reachable ? 'pointer' : 'default' }}
              ...
              <span className="step-label-desktop step-tab-label" style={{ fontSize: 10, fontWeight: 700, color: i + 1 <= step ? '#534AB7' : '#68629C', textDecoration: reachable ? 'underline' : 'none' }}>{s}</span>
              <span className="step-label-mobile step-tab-label" style={{ fontSize: 10, fontWeight: 700, color: i + 1 <= step ? '#534AB7' : '#68629C', textDecoration: reachable ? 'underline' : 'none' }}>{stepsMobile[i]}</span>
            </div>
          );
        })}
      </div>
