...
export default function SubmitClient() {
  ...
  const skip = (target: 4 | 5 = 4) => {
    ...
    if (target === 5) {
      if ((cnicFront && !cnicBack) || (!cnicFront && cnicBack)) {
        setVerifError(true);
        setError('Please upload both sides of the Candidate CNIC (front and back), or remove the single side before skipping.');
        setErrorField('cnic_front');
        return;
      }
      if ((guardianCnicFront && !guardianCnicBack) || (!guardianCnicFront && guardianCnicBack)) {
        setVerifError(true);
        setError('Please upload both sides of the Guardian CNIC (front and back), or remove the single side before skipping.');
        setErrorField('cnic_front');
        return;
      }
    }
