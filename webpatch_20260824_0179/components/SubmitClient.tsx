        ...
        <div style={{ border: `2px dashed ${file ? '#534AB7' : errorField === fieldKey ? '#DC2626' : '#E8E6F5'}`, borderRadius: 12, background: file ? '#EEEDFE' : errorField === fieldKey ? '#FEF2F2' : '#FAFAFA', overflow: 'hidden', height: 140, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          {preview
            ...
            : <div style={{ textAlign: 'center', color: '#68629C' }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 600, wordBreak: 'break-word', lineHeight: 1.3, padding: '0 4px' }}>Tap to upload {label.replace(' *', '')}</div>
...
export default function SubmitClient() {
  ...
  useEffect(() => {
    supabase.from('app_settings').select('key, value').then(({ data }) => {
      ...
      if (map['require_candidate_cnic_compulsory'] === 'false') setCompulsoryCandidateCnic(false);
      if (map['require_latest_degree_compulsory']  === 'true')  setCompulsoryLatestDegree(true);
      if (map['require_parents_cnic_compulsory']   === 'false') setCompulsoryParentsCnic(false);
    });
  }, []);
        ...
        {steps.map((s, i) => {
          ...
          return (
            <div key={i} style={{ flex: 1, textAlign: 'center', cursor: reachable ? 'pointer' : 'default' }}
              ...
              <span style={{ fontSize: 10, fontWeight: 700, color: i + 1 <= step ? '#534AB7' : '#68629C', textDecoration: reachable ? 'underline' : 'none', wordBreak: 'break-word', lineHeight: 1.2 }}>{stepsMobile[i]}</span>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E8E6F5', borderRadius: 20, padding: '28px' }}>
        ...
        {step === 4 && (
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <CnicUploadBox label={`CNIC Front${showCandidateCnic && compulsoryCandidateCnic ? " *" : ""}`} fieldKey="cnicFront" errorField={errorField}
              ...
              <CnicUploadBox label={`CNIC Back${showCandidateCnic && compulsoryCandidateCnic ? " *" : ""}`} fieldKey="cnicBack" errorField={errorField}
            ...
            {showLatestDegree && (
            <CnicUploadBox label={`Most Recent Education Document${showLatestDegree && compulsoryLatestDegree ? " *" : ""}`} fieldKey="educationDocument" errorField={errorField}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <CnicUploadBox label={`CNIC Front${showParentsCnic && compulsoryParentsCnic ? " *" : ""}`} fieldKey="guardianCnicFront" errorField={errorField}
              ...
              <CnicUploadBox label={`CNIC Back${showParentsCnic && compulsoryParentsCnic ? " *" : ""}`} fieldKey="guardianCnicBack" errorField={errorField}
              ...
              <div style={{ padding: 14, background: '#FEEDE3', border: '1px solid #E8620A66', borderRadius: 12, marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                  <input
                    ...
                    style={{ flex: 1, minWidth: 0, padding: '10px 12px', borderRadius: 8, border: 'none', fontSize: 14, fontWeight: 700, color: '#E8620A', letterSpacing: 2, outline: 'none', background: '#fff', boxSizing: 'border-box' }}
                  />
                  <button
                    type="button"
                    onClick={() => applyCoupon()}
                    ...
                    style={{
                      padding: '9px 14px', borderRadius: 8, border: 'none', flexShrink: 0, minWidth: 64, whiteSpace: 'nowrap',
              ...
              <div style={{ padding: 14, background: '#EEEDFE', border: '1px solid #D4D1F7', borderRadius: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                  <input
                    ...
                    style={{ flex: 1, minWidth: 0, padding: '10px 12px', borderRadius: 8, border: 'none', fontSize: 14, fontWeight: 700, color: '#534AB7', letterSpacing: 2, outline: 'none', background: '#fff', boxSizing: 'border-box' }}
                  />
                  <button
                    type="button"
