...
class SubscriptionScreen extends StatefulWidget {
  ...
  final void Function(bool isPaused)? onPauseStatusChanged;
  ...
  const SubscriptionScreen({super.key, this.onBack, this.onOpenCreateAccount, this.onOpenLogin, this.onPaidStatusChanged, this.onPauseStatusChanged, this.scrollToPlans = false, this.onOwnProfileEdited, this.onLoggedOut, this.onRenewTap, this.initialSelectedPlan = -1});
...
class SubscriptionScreenState extends State<SubscriptionScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _restoreActivationState().then((_) async {
      _refreshUserStatus();
      final proposalId = _userStatus?['id'] as String?;
      _subscribeToStatusChanges(proposalId);
      // Start realtime session monitoring for already-logged-in users
      final prefs = await SharedPreferences.getInstance();
      final sessionToken = prefs.getString('session_token');
      final cnicRaw = await _db.resolveEffectiveCnic();
      if (sessionToken != null && cnicRaw != null) {
        _subscribeToSessionChanges(cnicRaw.replaceAll('-', ''));
      }
    });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      ...
      // Realtime subscription handles session kick — no manual check needed
    }
  }
  ...
  int? _appliedCouponTrialDays;
  String? _appliedCouponCode;
  String? _couponMessage; // shown under the field — success or error text
  bool _couponIsError = false;
  bool _validatingCoupon = false;

  // Realtime session enforcement — instantly kicks this device when
  // another device logs in. No polling needed.
  void _subscribeToSessionChanges(String cnic) {
    _sessionChannel?.unsubscribe();
    _sessionChannel = Supabase.instance.client
      .channel('session:$cnic')
      .onPostgresChanges(
        ...
        callback: (payload) async {
          ...
          final isValid = await _db.checkDeviceSession(cnic, sessionToken);
          if (!isValid && mounted) {
            await showDialog(
              context: context,
              ...
              builder: (_) => AlertDialog(
                ...
                title: const Text('Logged Out', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: kInk)),
                content: const Text(
                  'You were logged out because your account was accessed from a new device. Only 1 device can be logged in at a time.',
                  style: TextStyle(fontSize: 13.5, color: kInkLight, height: 1.5),
                ),
                actions: [
                  TextButton(
        ...
        if (newStatus == 'deleted' && deletedFrom == 'permanent') {
          showDialog(
            ...
            builder: (_) => AlertDialog(
              backgroundColor: Colors.white,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
              title: const Text('Account Deleted!', style: TextStyle(fontSize: 15.5, fontWeight: FontWeight.w800, color: kInk)),
              content: const Text(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                            ...
                            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              Expanded(
                                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Row(children: [
                                    Flexible(
                                      child: Text(name, style: TextStyle(fontSize: s.f(14), fontWeight: FontWeight.w800, color: kInk), maxLines: 1, overflow: TextOverflow.ellipsis),
                                    ),
                                    // Same badge shown on the card in the
                                    // feed — not admin view (that's a
                                    // view mode, not a real verified
                                    // profile).
                                    if (!_isAdminView && _userStatus?['cnic_verified'] == true) ...[
                                      SizedBox(width: s.s(4)),
                                      Icon(Icons.verified_rounded, size: s.f(15), color: kPurple),
                                    ],
                                    if (!_isAdminView &&
                                        _userStatus?['is_doc_verified'] == true &&
                                        SupabaseService.instance.cachedSettings['verification_badge_enabled'] != 'false') ...[
                                      SizedBox(width: s.s(4)),
                                      Icon(Icons.verified_rounded, size: s.f(15), color: kGreen),
                                    ],
                                  ]),
                          ...
                          // ── Verify Now ── shown alongside Pay Now while
                          // the profile is still pending (matches the
                          // website, which shows this next to Pay Now for
                          // a pending, not-yet-verified profile), and also
                          // once the profile is unlocked/active — the
                          // original condition here, kept as-is. Still
                          // hidden for Rejected/Removed/Inactive (nothing
                          // real to verify in those states) and for the
                          // admin view.
                          if (!_isAdminView && !isInactive &&
                            // Hide if already verified
                            (_userStatus?['cnic_verified'] != true) &&

                            // Show Verify Now when:
                            // - any doc is rejected (must allow resubmit), OR
                            // - not yet verified AND any enabled section has no doc uploaded
                            // (even if a pending request exists for other docs, still show
                            //  if an enabled section is completely missing its upload)
                            (() {
                              final dv = (_userStatus?['doc_verification'] as Map<String, dynamic>? ?? {});
                              final anyRejected = dv.values.any((v) => v == 'rejected');
                              if (anyRejected) return true;
                              final isDocVerified = _userStatus?['is_doc_verified'] == true;
                              if (isDocVerified) return false;
                              final settings = SupabaseService.instance.cachedSettings;
                              final showCnic    = settings['verify_now_candidate_cnic'] != 'false';
                              final showDegree  = settings['verify_now_latest_degree']  != 'false';
                              final showParents = settings['verify_now_parents_cnic']   != 'false';
                              final hasCnic    = (_userStatus?['cnic_front_url'] as String? ?? '').isNotEmpty &&
                                                 (_userStatus?['cnic_back_url']  as String? ?? '').isNotEmpty;
                              final hasDegree  = (_userStatus?['education_document_url'] as String? ?? '').isNotEmpty;
                              final hasParents = (_userStatus?['guardian_cnic_front_url'] as String? ?? '').isNotEmpty &&
                                                 (_userStatus?['guardian_cnic_back_url']  as String? ?? '').isNotEmpty;
                              return (showCnic    && !hasCnic)
                                  || (showDegree  && !hasDegree)
                                  || (showParents && !hasParents);
                            }()) &&
                            // Show for pending AND active profiles (active users can still earn badge)
                            (isPending || !isLocked || _userStatus?['status'] == 'active'))
                            Padding(
                              padding: EdgeInsets.fromLTRB(s.s(16), s.s(8), s.s(16), 0),
                              child: SizedBox(
                                width: double.infinity,
                                child: OutlinedButton.icon(
                                  onPressed: () => closeThen(_showVerifyNowSheet),
                                  icon: Icon(Icons.verified_rounded, size: s.d(16), color: kPurple),
                                  label: Text(
                                    () {
                                      final settings = SupabaseService.instance.cachedSettings;
                                      final badgeEnabled = settings['verification_badge_enabled'] != 'false';
                                      if (!badgeEnabled) return 'Verify Now';
                                      // Check if any missing doc is compulsory
                                      final hasCnic    = (_userStatus?['cnic_front_url'] as String? ?? '').isNotEmpty &&
                                                         (_userStatus?['cnic_back_url']  as String? ?? '').isNotEmpty;
                                      final hasDegree  = (_userStatus?['education_document_url'] as String? ?? '').isNotEmpty;
                                      final hasParents = (_userStatus?['guardian_cnic_front_url'] as String? ?? '').isNotEmpty &&
                                                         (_userStatus?['guardian_cnic_back_url']  as String? ?? '').isNotEmpty;
                                      // A rejected compulsory doc counts the same as missing —
                                      // the user must re-upload it, so the label should say 'Verify Now'.
                                      final dv2 = (_userStatus?['doc_verification'] as Map<String, dynamic>? ?? {});
                                      final cnicRejected    = dv2['cnic_front'] == 'rejected' || dv2['cnic_back'] == 'rejected';
                                      final degreeRejected  = dv2['education_document'] == 'rejected';
                                      final parentsRejected = dv2['guardian_cnic_front'] == 'rejected' || dv2['guardian_cnic_back'] == 'rejected';
                                      final missingCompulsory =
                                        (settings['verify_now_candidate_cnic'] != 'false' &&
                                         settings['verify_now_candidate_cnic_compulsory'] != 'false' && (!hasCnic || cnicRejected)) ||
                                        (settings['verify_now_latest_degree']  != 'false' &&
                                         settings['verify_now_latest_degree_compulsory']  == 'true'  && (!hasDegree || degreeRejected)) ||
                                        (settings['verify_now_parents_cnic']   != 'false' &&
                                         settings['verify_now_parents_cnic_compulsory']   != 'false' && (!hasParents || parentsRejected));
                                      return missingCompulsory ? 'Verify Now' : 'Get Verified Badge';
                                    }(),
                                    style: TextStyle(fontSize: s.f(12), fontWeight: FontWeight.w700, color: kPurple)),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: const Text('Your account has been deleted.', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
  ...
  void _goToRenew() {
    ...
    // Go straight to the duration picker — same as Pay Now.
    // No need to scroll to plans first; the user already knows they
    // want to renew, so skip the extra step.
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  Future<void> _togglePause() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text(isPaused ? 'Resume Profile?' : 'Pause Profile?',
            style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: kInk)),
        content: Text(
          isPaused
    ...
    try {
      ...
      _bumpCardVersion();
      // Immediately notify the feed to lock/unlock contacts
      final pauseVal = newStatus == 'paused';
      widget.onPauseStatusChanged?.call(pauseVal);
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('user_status', newStatus);
      // Notify MainShell on both pause and resume so the feed re-reads
      // _isPendingUser from prefs and locks/unlocks contacts immediately.
      widget.onPaidStatusChanged?.call();
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Failed: $e'),
  ...
  Future<void> _performLogout() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    // Remove this device's session using the stored token
    final cnicForSession = (await _db.resolveEffectiveCnic())?.replaceAll('-', '');
    final sessionToken = prefs.getString('session_token');
    if (cnicForSession != null && sessionToken != null) {
      await _db.removeDeviceSession(cnicForSession, sessionToken);
    }
  ...
  Future<void> _activateCode() async {
    ...
    // Returns a token we store locally. All future checks use this token.
    final prefs = await SharedPreferences.getInstance();
    final deviceId = prefs.getString('device_id') ?? 'unknown';
    debugPrint('[SESSION] registering: cnic=$digits deviceId=$deviceId');
    final sessionToken = await _db.registerDeviceSession(digits, deviceId, deviceType: 'app');
    if (sessionToken != null) {
      await prefs.setString('session_token', sessionToken);
      debugPrint('[SESSION] token stored: $sessionToken');
      // Start realtime session monitoring immediately after login
      _subscribeToSessionChanges(digits);
    }
    if (!mounted) return;
  ...
  String get _adminWhatsApp => _settings['whatsapp_number'] ?? '923287654333';
  ...
  List<_Plan> get _plans {
    ...
    _Plan(index: 1, name: 'Featured Profile', priceLabel: _ftPrice, duration: _ftDuration, tagline: 'Stand out. Get noticed.', color: kAmber, lightColor: kAmberLight, icon: Icons.bolt_rounded, isPopular: false,
      features: ['Schedule your Featured Date', 'Choose your Featured Location', 'Up to 5× more visibility', "$_ftDuration validity"],
      notIncluded: [],
        ...
        actions: [
          Padding(
            ...
            child: GestureDetector(
              ...
              child: Container(
                // Matches the header's Login/Register button proportions
                // exactly (padding, font size, corner radius).
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  SizedBox(width: s.s(6)),
                  Text('Contact Support', style: TextStyle(fontSize: s.f(11), fontWeight: FontWeight.w700, color: Colors.white)),
                ]),
              ),
            ),
          ),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _HowItWorksRow(icon: Icons.assignment_turned_in_rounded, color: kTeal, bg: kTealLight, title: 'Verified Profiles', subtitle: 'Thousands of local and overseas proposals'),
            ...
            Center(
              child: Column(
                ...
                children: [
                  ...
                  Text('App version ${_appVersion.isNotEmpty ? _appVersion : '...'}', style: TextStyle(fontSize: s.f(11), color: kInkFaint)),
                ],
              ),
            ),
            SizedBox(height: s.s(12)),
  ...
  String _stdPriceWithCoupon() {
    if (_appliedCouponTrialDays != null) return 'Free';
    ...
    try {
      ...
      setDialogState(() {
        ...
        if (type == 'free_trial' && trialDays != null) {
          _appliedCouponTrialDays = trialDays;
          _appliedCouponFreeDays = null;
          _appliedCouponDiscount = null;
          _couponMessage = 'Free trial activated — $trialDays days free access!';
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            GestureDetector(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  SvgPicture.asset('assets/icons/whatsapp.svg', width: 15, height: 15),
                  const SizedBox(width: 5),
                  const Text('Need help?', style: TextStyle(fontSize: 12, color: kInkLight, fontWeight: FontWeight.w600)),
                ]),
              ),
          ]),
          const SizedBox(height: 6),
  ...
  // ── Verify Now sheet ── pick + upload CNIC front/back, education
  // document, and guardian CNIC front/back, then submit for admin
  // review. This only ever creates a pending request — no badge or
  // status is granted automatically at any point, even once an admin
  // approves it and the photos land on the profile; that auto-grant
  // used to happen via a set_cnic_verified database trigger, which has
  // since been removed. CNIC front+back stay required (as before) —
  // the education document and guardian CNIC are optional additions,
  // matching the website's Verify Now modal and the registration form's
  // Verification step.
  Future<void> _showVerifyNowSheet() async {
    // Load which sections are required from admin settings
    final settings = await _db.fetchAppSettings();
    // Check what the user already uploaded AND wasn't rejected.
    // A rejected doc still has its URL but must be re-uploaded, so it
    // should NOT be treated as "already done".
    final docV = (_userStatus?['doc_verification'] as Map<String, dynamic>? ?? {})
        .map((k, v) => MapEntry(k, v as String? ?? 'pending'));
    final cnicFrontOk   = (_userStatus?['cnic_front_url']         as String? ?? '').isNotEmpty && docV['cnic_front']          != 'rejected';
    final cnicBackOk    = (_userStatus?['cnic_back_url']           as String? ?? '').isNotEmpty && docV['cnic_back']           != 'rejected';
    final guardFrontOk  = (_userStatus?['guardian_cnic_front_url'] as String? ?? '').isNotEmpty && docV['guardian_cnic_front'] != 'rejected';
    final guardBackOk   = (_userStatus?['guardian_cnic_back_url']  as String? ?? '').isNotEmpty && docV['guardian_cnic_back']  != 'rejected';
    final degreeOk      = (_userStatus?['education_document_url']  as String? ?? '').isNotEmpty && docV['education_document']  != 'rejected';

    final alreadyHasCandidateCnic = cnicFrontOk  && cnicBackOk;
    final alreadyHasDegree        = degreeOk;
    final alreadyHasParentsCnic   = guardFrontOk && guardBackOk;

    final requireCnic    = !alreadyHasCandidateCnic && settings['verify_now_candidate_cnic'] != 'false';
    final requireDegree  = !alreadyHasDegree        && settings['verify_now_latest_degree']  != 'false';
    final requireParents = !alreadyHasParentsCnic   && settings['verify_now_parents_cnic']   != 'false';
    final compulsoryCnic    = settings['verify_now_candidate_cnic_compulsory'] != 'false';
    final compulsoryDegree  = settings['verify_now_latest_degree_compulsory']  == 'true';
    final compulsoryParents = settings['verify_now_parents_cnic_compulsory']   != 'false';

    File? frontFile;
    File? backFile;
    File? educationFile;
    File? guardianFrontFile;
    File? guardianBackFile;
    bool uploading = false;
    String? errorMsg;
    final picker = ImagePicker();

    Future<void> pick(StateSetter setSheetState, ValueChanged<File> onPicked) async {
    ...
    await showModalBottomSheet(
      ...
      builder: (sheetCtx) => Padding(
        ...
        child: StatefulBuilder(
          builder: (sheetCtx, setSheetState) {
            final s = _S.of(sheetCtx);
            Widget slot(String label, File? file, VoidCallback onTap, {VoidCallback? onRemove}) {
              return GestureDetector(
                onTap: onTap,
                child: Container(
                  height: s.d(110),
                  ...
                  child: file != null
                      ? Stack(fit: StackFit.expand, children: [
                          ...
                          Positioned(
                            top: s.s(6), right: s.s(6),
                            child: GestureDetector(
                              onTap: onRemove,
                              child: Container(
                                width: s.d(24), height: s.d(24),
                                decoration: const BoxDecoration(color: Colors.black54, shape: BoxShape.circle),
                                child: Icon(Icons.close_rounded, color: Colors.white, size: s.d(14)),
                              ),
                            ),
                          ),
                        ])
                      : Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.add_a_photo_rounded, color: kPurple, size: s.d(24)),
                            SizedBox(height: s.s(6)),
                            label.endsWith(' *')
                                ? RichText(text: TextSpan(children: [
                                    TextSpan(text: label.replaceAll(' *', ''), style: TextStyle(fontSize: s.f(12), fontWeight: FontWeight.w700, color: kPurple)),
                                    TextSpan(text: ' *', style: TextStyle(fontSize: s.f(12), fontWeight: FontWeight.w700, color: kRose)),
                                  ]))
                                : Text(label, style: TextStyle(fontSize: s.f(12), fontWeight: FontWeight.w700, color: kPurple)),
                          ],
                        ),
                ),
              );
            ...
            Widget sectionLabel(String t) => Padding(
              padding: EdgeInsets.only(top: s.s(16), bottom: s.s(8)),
              child: Text(t, style: TextStyle(fontSize: s.f(11), fontWeight: FontWeight.w800, color: kInkFaint, letterSpacing: 0.6)),
            );

            return SafeArea(
              child: Container(
                padding: EdgeInsets.fromLTRB(s.s(20), s.s(20), s.s(20), s.s(20)),
                decoration: BoxDecoration(
                  color: kCardBg,
                  borderRadius: BorderRadius.vertical(top: Radius.circular(s.s(24))),
                ),
                child: SingleChildScrollView(
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    ...
                    children: [
                      Center(
                        child: Container(width: s.s(36), height: s.s(4),
                          decoration: BoxDecoration(color: kBorder, borderRadius: BorderRadius.circular(s.s(2)))),
                      ),
                      SizedBox(height: s.s(18)),
                      Row(children: [
                        Container(width: s.d(40), height: s.d(40),
                          decoration: BoxDecoration(color: kPurpleLight, borderRadius: BorderRadius.circular(s.s(12))),
                          child: Icon(Icons.verified_rounded, color: kPurple, size: s.d(20))),
                        SizedBox(width: s.s(12)),
                        Expanded(
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      if (requireCnic) ...[
                        sectionLabel('FOR MARRIAGE-SEEKING PERSON'),
                        Row(children: [
                          Expanded(child: slot(compulsoryCnic ? 'CNIC Front *' : 'CNIC Front', frontFile, () => pick(setSheetState, (f) => frontFile = f), onRemove: frontFile != null ? () => setSheetState(() => frontFile = null) : null)),
                          SizedBox(width: s.s(10)),
                          Expanded(child: slot(compulsoryCnic ? 'CNIC Back *' : 'CNIC Back', backFile, () => pick(setSheetState, (f) => backFile = f), onRemove: backFile != null ? () => setSheetState(() => backFile = null) : null)),
                        ]),
                        SizedBox(height: s.s(10)),
                      ],
                      if (requireDegree) ...[
                        if (!requireCnic) sectionLabel('MARRIAGE-SEEKING CANDIDATE'),
                        SizedBox(width: double.infinity, child: slot(compulsoryDegree ? 'Recent Education Document *' : 'Recent Education Document', educationFile, () => pick(setSheetState, (f) => educationFile = f), onRemove: educationFile != null ? () => setSheetState(() => educationFile = null) : null)),
                        SizedBox(height: s.s(10)),
                      ],
                      if (requireParents) ...[
                        sectionLabel('PARENT / GUARDIAN'),
                        Row(children: [
                          Expanded(child: slot(compulsoryParents ? 'CNIC Front *' : 'CNIC Front', guardianFrontFile, () => pick(setSheetState, (f) => guardianFrontFile = f), onRemove: guardianFrontFile != null ? () => setSheetState(() => guardianFrontFile = null) : null)),
                          SizedBox(width: s.s(10)),
                          Expanded(child: slot(compulsoryParents ? 'CNIC Back *' : 'CNIC Back', guardianBackFile, () => pick(setSheetState, (f) => guardianBackFile = f), onRemove: guardianBackFile != null ? () => setSheetState(() => guardianBackFile = null) : null)),
                        ]),
                        SizedBox(height: s.s(10)),
                      ],
                      SizedBox(height: s.s(10)),
                      Text(
                        'Your documents stay private — visible only to our review team, never shown on your profile.',
                        style: TextStyle(fontSize: s.f(11), color: kInkFaint, height: 1.5),
                      ),
                      if (errorMsg != null) ...[
                        SizedBox(height: s.s(10)),
                        Text(errorMsg!, style: TextStyle(fontSize: s.f(12), color: kRose, fontWeight: FontWeight.w600)),
                      ],
                      SizedBox(height: s.s(18)),
                      GestureDetector(
                        onTap: uploading ? null : () async {
                          if (requireCnic && (frontFile == null || backFile == null)) {
                            setSheetState(() => errorMsg = 'Please add both CNIC Front and Back photos');
                            return;
                          }
                          if (!requireCnic && !requireDegree && !requireParents) {
                            setSheetState(() => errorMsg = 'No verification documents are currently required');
                            return;
                          }
                          // All compulsory sections must be uploaded together
                          final missing = <String>[];
                          if (requireCnic && compulsoryCnic && (frontFile == null || backFile == null)) {
                            missing.add('Candidate CNIC (front & back)');
                          }
                          if (requireDegree && compulsoryDegree && educationFile == null) {
                            missing.add('Recent Education Document');
                          }
                          if (requireParents && compulsoryParents && (guardianFrontFile == null || guardianBackFile == null)) {
                            missing.add('Parent/Guardian CNIC (front & back)');
                          }
                          if (missing.isNotEmpty) {
                            setSheetState(() => errorMsg = 'Please upload all required documents: ${missing.join(', ')}');
                            return;
                          }
                          // Pair enforcement: guardian CNIC must be both sides or neither
                          if ((guardianFrontFile != null && guardianBackFile == null) ||
                              (guardianFrontFile == null && guardianBackFile != null)) {
                            setSheetState(() => errorMsg = 'Please add both Parent/Guardian CNIC Front and Back photos');
                            return;
                          }
                          setSheetState(() { uploading = true; errorMsg = null; });
                          try {
                            debugPrint('[VERIFY] Starting submission...');
                            debugPrint('[VERIFY] requireCnic=$requireCnic requireDegree=$requireDegree requireParents=$requireParents');
                            
                            final cnic = await _db.resolveEffectiveCnic();
                            debugPrint('[VERIFY] cnic=$cnic');
                            if (cnic == null || cnic.isEmpty) throw Exception('Could not verify account — CNIC is null or empty');

                            // Only upload sections that are required
                            String? frontUrl;
                            String? backUrl;
                            if (requireCnic && frontFile != null && backFile != null) {
                              debugPrint('[VERIFY] Uploading CNIC front...');
                              frontUrl = await _db.uploadVerificationCnic(await frontFile!.readAsBytes(), side: 'front', cnic: cnic);
                              debugPrint('[VERIFY] frontUrl=$frontUrl');
                              debugPrint('[VERIFY] Uploading CNIC back...');
                              backUrl = await _db.uploadVerificationCnic(await backFile!.readAsBytes(), side: 'back', cnic: cnic);
                              debugPrint('[VERIFY] backUrl=$backUrl');
                            }
                            String? educationUrl;
                            if (requireDegree && educationFile != null) {
                              debugPrint('[VERIFY] Uploading education doc...');
                              educationUrl = await _db.uploadVerificationEducationDocument(await educationFile!.readAsBytes(), cnic: cnic);
                              debugPrint('[VERIFY] educationUrl=$educationUrl');
                            }
                            String? guardianFrontUrl;
                            String? guardianBackUrl;
                            if (requireParents) {
                              if (guardianFrontFile != null) {
                                debugPrint('[VERIFY] Uploading guardian front...');
                                guardianFrontUrl = await _db.uploadVerificationGuardianCnic(await guardianFrontFile!.readAsBytes(), side: 'front', cnic: cnic);
                                debugPrint('[VERIFY] guardianFrontUrl=$guardianFrontUrl');
                              }
                              if (guardianBackFile != null) {
                                debugPrint('[VERIFY] Uploading guardian back...');
                                guardianBackUrl = await _db.uploadVerificationGuardianCnic(await guardianBackFile!.readAsBytes(), side: 'back', cnic: cnic);
                                debugPrint('[VERIFY] guardianBackUrl=$guardianBackUrl');
                              }
                            }
                            debugPrint('[VERIFY] Calling submitCnicVerification...');
                            final ok = await _db.submitCnicVerification(
                              cnic: cnic, frontUrl: frontUrl, backUrl: backUrl,
                              educationDocumentUrl: educationUrl,
                              guardianFrontUrl: guardianFrontUrl,
                              guardianBackUrl: guardianBackUrl,
                            );
                            debugPrint('[VERIFY] submitCnicVerification result=$ok');
                            if (!ok) throw Exception('Submission RPC returned false');
                            if (sheetCtx.mounted) Navigator.pop(sheetCtx);
                            // Refresh user status so the Verify Now button disappears immediately
                            _refreshUserStatus();
                            if (mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Submitted! Our team will review your documents shortly.'), backgroundColor: kInk),
                              );
                            }
                          } catch (e, st) {
                            debugPrint('[VERIFY] ERROR: $e');
                            debugPrint('[VERIFY] STACK: $st');
                            setSheetState(() { uploading = false; errorMsg = "Couldn't submit: $e"; });
                          }
                        },
                        child: Container(
                          width: double.infinity,
                          ...
                          decoration: BoxDecoration(
                            gradient: uploading ? null : const LinearGradient(colors: [kPurple, kPurpleDeep]),
                            color: uploading ? kBorder : null,
                            borderRadius: BorderRadius.circular(s.s(14)),
                          ),
                          child: Center(
                            child: uploading
                                ? SizedBox(width: s.d(18), height: s.d(18), child: const CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                                : Text('Submit for Review', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: s.f(15))),
                          ),
                        ),
                      ),
                    ],
