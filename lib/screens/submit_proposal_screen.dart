...
class _SubmitProposalScreenState extends State<SubmitProposalScreen> {
  ...
  File? _educationDocumentFile, _guardianCnicFrontFile, _guardianCnicBackFile;
  // String flags for UI
  String? _profilePhoto, _cnicFront, _cnicBack;
  String? _educationDocument, _guardianCnicFront, _guardianCnicBack;
  ...
  void _saveDraftSync() {
    ...
    final data = {
      ...
      'educationDocumentPath': _educationDocumentFile?.path,
      'guardianCnicFrontPath': _guardianCnicFrontFile?.path,
      'guardianCnicBackPath': _guardianCnicBackFile?.path,
      ...
      'maritalStatus': _maritalStatus, 'practiceLevel': _practiceLevel,
  ...
  void _restoreDraft(SharedPreferences prefs) {
    ...
    try {
      ...
      setState(() {
        ...
        final educationDocPath = data['educationDocumentPath'] as String?;
        final guardianFrontPath = data['guardianCnicFrontPath'] as String?;
        final guardianBackPath = data['guardianCnicBackPath'] as String?;
        if (educationDocPath != null && File(educationDocPath).existsSync()) {
          _educationDocumentFile = File(educationDocPath);
          _educationDocument = 'selected';
        }
        if (guardianFrontPath != null && File(guardianFrontPath).existsSync()) {
          _guardianCnicFrontFile = File(guardianFrontPath);
          _guardianCnicFront = 'selected';
        }
        if (guardianBackPath != null && File(guardianBackPath).existsSync()) {
          _guardianCnicBackFile = File(guardianBackPath);
          _guardianCnicBack = 'selected';
        }
  ...
  // True when admin has turned off all 3 verification sections.
  // When true, skip step 2 entirely — hide its tab and jump over it.
  bool get _noVerifSections {
    final s = SupabaseService.instance.cachedSettings;
    return s['require_candidate_cnic'] == 'false'
        && s['require_latest_degree']  == 'false'
        && s['require_parents_cnic']   == 'false';
  }
  ...
  String? _validateStep2() {
    ...
    //
    // Partial CNIC pairs are still caught regardless
    if (_cnicFront != null && _cnicBack == null) { _invalidField = 'cnicBack'; return 'Please also add the CNIC Back photo'; }
    if (_cnicBack != null && _cnicFront == null) { _invalidField = 'cnicFront'; return 'Please also add the CNIC Front photo'; }
    if (_guardianCnicFront != null && _guardianCnicBack == null) { _invalidField = 'guardianCnicBack'; return 'Please also add the Guardian CNIC Back photo'; }
    if (_guardianCnicBack != null && _guardianCnicFront == null) { _invalidField = 'guardianCnicFront'; return 'Please also add the Guardian CNIC Front photo'; }

    // If verification is compulsory, check each required doc type
    final settings = SupabaseService.instance.cachedSettings;
    if (settings['require_verification_step'] == 'true') {
      final missing = <String>[];
      if (settings['require_candidate_cnic'] != 'false' && (_cnicFront == null || _cnicBack == null)) {
        missing.add('Candidate CNIC (front & back)');
      }
      if (settings['require_latest_degree'] != 'false' && _educationDocument == null) {
        missing.add('Education Document');
      }
      if (settings['require_parents_cnic'] != 'false' && (_guardianCnicFront == null || _guardianCnicBack == null)) {
        missing.add('Guardian CNIC (front & back)');
      }
      if (missing.isNotEmpty) {
        _invalidField = 'cnicFront';
        return 'Please upload all required documents: ${missing.join(', ')}';
      }
    }
  ...
  Widget _buildStepIndicator() {
    final s = _S.of(context);
    final visibleSteps = _noVerifSections
        ? _steps.where((s) => s != 'Verification').toList()
        : _steps;
    // Map visible-step indices back to real step indices for highlighting.
    final visibleIndices = _noVerifSections
        ? [0, 1, 3]  // skip step 2
        : [0, 1, 2, 3];
    return Container(
      color: kCardBg,
      ...
      child: Row(
        children: visibleSteps.asMap().entries.map((e) {
          final realIdx = visibleIndices[e.key];
          final active = _step == realIdx;
          final done = _maxStep > realIdx;
          final reachable = realIdx <= _maxStep && !active;
          return Expanded(child: Row(children: [
            Expanded(child: GestureDetector(
              onTap: reachable ? () => setState(() => _step = realIdx) : null,
            ...
            if (e.key < visibleSteps.length - 1) SizedBox(width: s.s(4)),
          ]));
        }).toList(),
      ),
    );
  ...
  Widget _buildStep(int step) {
    switch (step) {
      ...
      // Step 2 is Verification. If all sections are off, redirect to step 3.
      case 2:
        if (_noVerifSections) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted) setState(() { _step = 3; if (_step > _maxStep) _maxStep = _step; });
          });
        ...
        onCnicFrontRemove: () => setState(() { _cnicFrontFile = null; _cnicFront = null; }),
        onCnicBackRemove: () => setState(() { _cnicBackFile = null; _cnicBack = null; }),
        educationDocumentFile: _educationDocumentFile,
        educationDocument: _educationDocument,
        onEducationDocumentTap: (source) async {
          final f = await _pickImage(source);
          if (f != null) setState(() { _educationDocumentFile = f; _educationDocument = 'selected'; });
        },
        onEducationDocumentRemove: () => setState(() { _educationDocumentFile = null; _educationDocument = null; }),
        guardianCnicFrontFile: _guardianCnicFrontFile, guardianCnicBackFile: _guardianCnicBackFile,
        guardianCnicFront: _guardianCnicFront, guardianCnicBack: _guardianCnicBack,
        onGuardianCnicFrontTap: (source) async {
          final f = await _pickImage(source);
          if (f != null) setState(() { _guardianCnicFrontFile = f; _guardianCnicFront = 'selected'; });
        },
        onGuardianCnicBackTap: (source) async {
          final f = await _pickImage(source);
          if (f != null) setState(() { _guardianCnicBackFile = f; _guardianCnicBack = 'selected'; });
        },
        onGuardianCnicFrontRemove: () => setState(() { _guardianCnicFrontFile = null; _guardianCnicFront = null; }),
        onGuardianCnicBackRemove: () => setState(() { _guardianCnicBackFile = null; _guardianCnicBack = null; }),
        onSkip: () => setState(() { _step++; if (_step > _maxStep) _maxStep = _step; _invalidField = null; _verifError = false; }),
        invalidField: _invalidField,
        verifError: _verifError,
      );
      case 3: return _ReviewStep(
        ...
        educationDocumentFile: _educationDocumentFile,
        guardianCnicFrontFile: _guardianCnicFrontFile,
        guardianCnicBackFile: _guardianCnicBackFile,
      );
          ...
          child: Row(
            children: [
              if (_step > 0) ...[
                Expanded(
                  child: PressButton(
                    onTap: _submitting ? () {} : () => setState(() => _step = (_noVerifSections && _step == 3) ? 1 : _step - 1),
                    child: Container(
  ...
  void _onNext() {
    ...
    setState(() {
      ...
      if (_step == 2 && error != null) _verifError = true;
      if (_step == 2 && error == null) _verifError = false;
    });
    if (error != null) {
    ...
    // When all verification sections are off, jump from step 1 → step 3
    // (skipping step 2) and from step 3 back to step 1 on back.
    final nextStep = (_noVerifSections && _step == 1) ? 3 : _step + 1;
    if (_step < _steps.length - 1) {
      setState(() { _step = nextStep; if (_step > _maxStep) _maxStep = _step; });
      _saveDraftSync();
    } else {
      // Submitter type is compulsory — adminNotesCtrl is written by _SubmitterTypeSelector
      if (adminNotesCtrl.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Row(children: [
            const Icon(Icons.info_outline_rounded, color: Colors.white, size: 18),
            const SizedBox(width: 10),
            const Expanded(child: Text('Please select one of the options above before submitting.', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600))),
          ]),
          backgroundColor: const Color(0xFFE05C5C),
          behavior: SnackBarBehavior.floating,
          margin: EdgeInsets.fromLTRB(16, 0, 16, MediaQuery.of(context).padding.bottom + 50),
  ...
  Future<void> _submitProposal() async {
    // CNIC photos are optional now — this used to hard-block submission
    // if both weren't present, which contradicted making them optional
    // in the first place. The step-level validator (_validateStep2)
    // already handles the one real remaining rule (if you start
    // uploading one side, finish the pair).
    ...
    try {
      ...
      'looking_for': _lookingForCtrl.text.trim().isEmpty ? null : _lookingForCtrl.text.trim(),
        ...
        // cnic_verified deliberately NOT sent here. It used to be decided
        // by a database trigger (set_cnic_verified) based on whether both
        // CNIC photos were present, but that trigger has been removed —
        // verified badges are no longer auto-assigned from having
        // documents on file at all, so this always stays whatever the
        // column defaults to.
  ...
  // Country-aware phone grouping — same table as phoneDisplay() in supabase.ts
  ...
  String _formatDialedPhone(String dialCode, String number) {
    ...
    final fmt = _phoneFormats.where((f) => f.$1 == dialCode).firstOrNull;
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('Thank you for submitting your profile. Please log in to your account and complete the verification process.', textAlign: TextAlign.center, style: TextStyle(fontSize: s.f(13), color: kInkLight, height: 1.55)),
          SizedBox(height: s.s(16)),
...
class _BasicInfoStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: EdgeInsets.all(s.s(20)), children: [
      ...
      SizedBox(height: s.s(12)),
      SearchableGroupedDropdown(label: 'Caste *', value: caste, groups: SupabaseService.instance.castesGrouped, onChanged: onCasteChanged, icon: Icons.people_outline_rounded, isInvalid: invalidField == 'caste', pinnedOption: (label: "Other (my caste isn't listed)", value: 'Other')),
      if (caste == 'Other') ...[
        Container(
          margin: EdgeInsets.only(top: s.s(8), left: s.s(16)),
          padding: EdgeInsets.all(s.s(12)),
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (hasKids == 'Yes') ...[
              ...
              Row(children: [
                Expanded(child: _AppTextField(controller: boysCtrl, label: 'Sons', hint: '0', isNumber: true, maxLength: 2, showCounter: false, inputFormatters: [FilteringTextInputFormatter.digitsOnly, RangeInputFormatter(min: 0, max: 20, maxDigits: 2)])),
                const SizedBox(width: 12),
                Expanded(child: _AppTextField(controller: girlsCtrl, label: 'Daughters', hint: '0', isNumber: true, maxLength: 2, showCounter: false, inputFormatters: [FilteringTextInputFormatter.digitsOnly, RangeInputFormatter(min: 0, max: 20, maxDigits: 2)])),
              ]),
            ],
          ]),
        ),
    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
      ...
      if (hasSiblings == 'Yes') ...[
        Container(
          ...
          child: Row(children: [
            Expanded(child: _AppTextField(controller: brothersCtrl, label: 'Brothers', hint: '0', isNumber: true, maxLength: 2, showCounter: false, inputFormatters: [FilteringTextInputFormatter.digitsOnly, RangeInputFormatter(min: 0, max: 20, maxDigits: 2)])),
            const SizedBox(width: 12),
            Expanded(child: _AppTextField(controller: sistersCtrl, label: 'Sisters', hint: '0', isNumber: true, maxLength: 2, showCounter: false, inputFormatters: [FilteringTextInputFormatter.digitsOnly, RangeInputFormatter(min: 0, max: 20, maxDigits: 2)])),
          ]),
        ),
      ],
      _sec('EDUCATION'),
...
class _VerificationStep extends StatefulWidget {
  ...
  final VoidCallback? onCnicFrontRemove, onCnicBackRemove;
  final File? educationDocumentFile;
  final String? educationDocument;
  final ValueChanged<String> onEducationDocumentTap;
  final VoidCallback? onEducationDocumentRemove;
  final File? guardianCnicFrontFile, guardianCnicBackFile;
  final String? guardianCnicFront, guardianCnicBack;
  final ValueChanged<String> onGuardianCnicFrontTap, onGuardianCnicBackTap;
  final VoidCallback? onGuardianCnicFrontRemove, onGuardianCnicBackRemove;
  final VoidCallback onSkip;
  final String? invalidField;
  final bool verifError;
  final String? skipError; // custom message when Skip is blocked
  ...
  const _VerificationStep({
    ...
    this.onCnicFrontRemove, this.onCnicBackRemove,
    this.educationDocumentFile,
    this.educationDocument,
    required this.onEducationDocumentTap,
    this.onEducationDocumentRemove,
    this.guardianCnicFrontFile, this.guardianCnicBackFile,
    this.guardianCnicFront, this.guardianCnicBack,
    required this.onGuardianCnicFrontTap,
    required this.onGuardianCnicBackTap,
    this.onGuardianCnicFrontRemove, this.onGuardianCnicBackRemove,
    required this.onSkip,
    this.invalidField,
    this.verifError = false,
    this.skipError,
  });
...
class _VerificationStepState extends State<_VerificationStep> {
  ...
  Widget _sec(String t, {bool required = false}) => Padding(
    padding: const EdgeInsets.only(top: 22, bottom: 10),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Text(t, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: kInkFaint, letterSpacing: 0.8)),
        if (required) const Text(' *', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: kRose)),
      ]),
      const SizedBox(height: 6),
      Divider(height: 1, color: kBorder),
    ]),
  ...
  Widget build(BuildContext context) {
    return ListView(padding: const EdgeInsets.all(20), children: [
      Row(children: [
        Expanded(child: _FormTitle(
          title: 'Verification',
          subtitle: 'Your documents remain private and fully secured',
          icon: Icons.verified_user_outlined,
          iconColor: kPurple,
          iconBg: kPurpleLight,
        )),
        if (SupabaseService.instance.cachedSettings['require_verification_step'] != 'true')
          GestureDetector(
            onTap: () {
              // Pair enforcement: block skip if one CNIC side is uploaded without the other.
              // Does NOT turn the info box red — just shows a snackbar.
              final hasCnicFront  = (widget.cnicFront  ?? '').isNotEmpty;
              final hasCnicBack   = (widget.cnicBack   ?? '').isNotEmpty;
              final hasGuardFront = (widget.guardianCnicFront ?? '').isNotEmpty;
              final hasGuardBack  = (widget.guardianCnicBack  ?? '').isNotEmpty;
              if ((hasCnicFront && !hasCnicBack) || (!hasCnicFront && hasCnicBack)) {
                setState(() => _skipError = 'Please upload both sides of the Candidate CNIC (front and back), or remove the single side before skipping.');
                return;
              }
              if ((hasGuardFront && !hasGuardBack) || (!hasGuardFront && hasGuardBack)) {
                setState(() => _skipError = 'Please upload both sides of the Parent/Guardian CNIC (front and back), or remove the single side before skipping.');
                return;
              }
      ...
      // Identity verification documents are required to activate the
      // account, but every one of them is skippable and addable later —
      // no badge or status is promised here, since that's no longer
      // assigned just from having documents on file.
      Container(
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          color: (_skipError != null || widget.verifError) ? const Color(0xFFFEE2E2) : kPurpleLight,
          borderRadius: BorderRadius.circular(12),
        ),
        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Icon(Icons.info_outline_rounded, color: (_skipError != null || widget.verifError) ? kRose : kPurple, size: 20),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
              _skipError != null
                ? _skipError!
                : SupabaseService.instance.cachedSettings['require_verification_step'] == 'true'
                  ? 'The following documents are required to verify your identity.'
                  : (() {
                      final st = SupabaseService.instance.cachedSettings;
                      final anyCompulsory =
                        (st['require_candidate_cnic'] != 'false' && st['require_candidate_cnic_compulsory'] != 'false') ||
                        (st['require_latest_degree']  != 'false' && st['require_latest_degree_compulsory']  == 'true') ||
                        (st['require_parents_cnic']   != 'false' && st['require_parents_cnic_compulsory']   != 'false');
                      return anyCompulsory
                        ? 'The following documents are required to verify your identity.'
                        : 'Submit the following documents to get a verified badge.';
                    }()),
              style: TextStyle(fontSize: 12.5, color: (_skipError != null || widget.verifError) ? kRose : kInk, height: 1.5, fontWeight: FontWeight.w500),
            ),
          ),
        ]),
      ),

      if (SupabaseService.instance.cachedSettings['require_candidate_cnic'] != 'false') ...[
        _sec('FOR MARRIAGE-SEEKING PERSON'),
        Row(children: [
        ...
        _FullPhotoSlot(
          label: (SupabaseService.instance.cachedSettings['require_candidate_cnic_compulsory'] != 'false' && SupabaseService.instance.cachedSettings['require_candidate_cnic'] != 'false') ? 'CNIC Front *' : 'CNIC Front',
          ...
          onRemove: widget.onCnicFrontRemove,
        ),
        const SizedBox(height: 16),
        Row(children: [
        ...
        const SizedBox(height: 8),
        _FullPhotoSlot(
          label: (SupabaseService.instance.cachedSettings['require_candidate_cnic_compulsory'] != 'false' && SupabaseService.instance.cachedSettings['require_candidate_cnic'] != 'false') ? 'CNIC Back *' : 'CNIC Back',
          ...
          onRemove: widget.onCnicBackRemove,
        ),
      ],

      if (SupabaseService.instance.cachedSettings['require_latest_degree'] != 'false') ...[
        const SizedBox(height: 16),
        Text((SupabaseService.instance.cachedSettings['require_latest_degree_compulsory'] == 'true' && SupabaseService.instance.cachedSettings['require_latest_degree'] != 'false') ? 'Most Recent Education Document *' : 'Most Recent Education Document', style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: kInkLight)),
        const SizedBox(height: 8),
        _FullPhotoSlot(
          label: 'Education Document',
          file: widget.educationDocumentFile,
          selected: widget.educationDocument != null,
          onSourceSelected: widget.onEducationDocumentTap,
          onRemove: widget.onEducationDocumentRemove,
        ),
      ],

      if (SupabaseService.instance.cachedSettings['require_parents_cnic'] != 'false') ...[
        _sec('PARENT / GUARDIAN'),
        Row(children: [
          const Text('CNIC Front', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: kInkLight)),
          if (widget.invalidField == 'guardianCnicFront') ...[const SizedBox(width: 6), const Icon(Icons.error_rounded, color: kRose, size: 14)],
        ]),
        const SizedBox(height: 8),
        _FullPhotoSlot(
          label: (SupabaseService.instance.cachedSettings['require_parents_cnic_compulsory'] != 'false' && SupabaseService.instance.cachedSettings['require_parents_cnic'] != 'false') ? 'Parent CNIC Front *' : 'Parent CNIC Front',
          file: widget.guardianCnicFrontFile,
          selected: widget.guardianCnicFront != null,
          onSourceSelected: widget.onGuardianCnicFrontTap,
          isInvalid: widget.invalidField == 'guardianCnicFront',
          onRemove: widget.onGuardianCnicFrontRemove,
        ),
        const SizedBox(height: 16),
        Row(children: [
          const Text('CNIC Back', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: kInkLight)),
          if (widget.invalidField == 'guardianCnicBack') ...[const SizedBox(width: 6), const Icon(Icons.error_rounded, color: kRose, size: 14)],
        ]),
        const SizedBox(height: 8),
        _FullPhotoSlot(
          label: (SupabaseService.instance.cachedSettings['require_parents_cnic_compulsory'] != 'false' && SupabaseService.instance.cachedSettings['require_parents_cnic'] != 'false') ? 'Parent CNIC Back *' : 'Parent CNIC Back',
          file: widget.guardianCnicBackFile,
          selected: widget.guardianCnicBack != null,
          onSourceSelected: widget.onGuardianCnicBackTap,
          isInvalid: widget.invalidField == 'guardianCnicBack',
          onRemove: widget.onGuardianCnicBackRemove,
        ),
      ],
      const SizedBox(height: 30),
    ]);
...
class _FullPhotoSlot extends StatelessWidget {
  ...
  const _FullPhotoSlot({required this.label, required this.selected, this.file, required this.onSourceSelected, this.isInvalid = false, this.onRemove});
      ...
      child: Container(
        ...
        child: file != null
            ? ClipRRect(
                ...
                child: Stack(fit: StackFit.expand, children: [
                  Image.file(file!, fit: BoxFit.cover),
                  // Replaces what used to be a static purple checkmark
                  // here — this is now an actual remove button, letting
                  // someone clear a wrongly uploaded photo and get back
                  // to the "tap to upload" state directly, without it
                  // just confirming a photo was selected with no way to
                  // undo that from this slot itself.
                  Positioned(top: s.s(8), right: s.s(8),
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Icon(Icons.close_rounded, color: Colors.white, size: s.d(16)),
                      ),
                    ),
                  ),
            ...
            : Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                label.endsWith(' *')
                  ? RichText(text: TextSpan(children: [
                      TextSpan(text: label.replaceAll(' *', ''), style: TextStyle(fontSize: s.f(13), fontWeight: FontWeight.w700, color: kInkFaint)),
                      TextSpan(text: ' *', style: TextStyle(fontSize: s.f(13), fontWeight: FontWeight.w700, color: kRose)),
                    ]))
...
class _ReviewStep extends StatelessWidget {
  ...
  final File? educationDocumentFile, guardianCnicFrontFile, guardianCnicBackFile;
  ...
  const _ReviewStep({
    ...
    this.educationDocumentFile, this.guardianCnicFrontFile, this.guardianCnicBackFile,
  });
    ...
    return ListView(padding: EdgeInsets.all(s.s(20)), children: [
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          // ── VERIFICATION ──
          // Previously mixed into Basic Info instead of having its own
          // section — moved out to match the website's review step,
          // which groups these under their own "VERIFICATION" heading.
          if ((cnic != null && cnic!.isNotEmpty) || cnicFrontFile != null || cnicBackFile != null ||
              educationDocumentFile != null || guardianCnicFrontFile != null || guardianCnicBackFile != null) ...[
            _secDark(context, 'VERIFICATION'),
            ...
            if (educationDocumentFile != null) _R('Education Document', ' ', file: educationDocumentFile),
            if (guardianCnicFrontFile != null) _R('Guardian CNIC Front', ' ', file: guardianCnicFrontFile),
            if (guardianCnicBackFile != null) _R('Guardian CNIC Back', ' ', file: guardianCnicBackFile),
          ],
        ]),
      ),
      SizedBox(height: s.s(16)),
      // ── Coupon & Referral dropdown (above checkboxes) ──
      if (couponCtrl != null || affiliateCtrl != null) ...[
        _CodesDropdown(couponCtrl: couponCtrl, affiliateCtrl: affiliateCtrl),
        SizedBox(height: s.s(16)),
      ],
      // ── Submitter Type — writes selection to adminNotesCtrl so admin sees it
      _SubmitterTypeSelector(
        initialValue: adminNotesCtrl.text.contains('parent') ? 'guardian' : adminNotesCtrl.text.contains('self') ? 'self' : null,
        onChanged: (v) {
          adminNotesCtrl.text = v == 'self'
              ? 'Profile submitted for self'
              : 'Profile submitted by parent/guardian';
        },
      ),
...
class _FormTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
      ...
      SizedBox(width: s.s(12)),
      // Expanded + ellipsis so a long subtitle shrinks to fit instead of
      // overflowing when a sibling (e.g. a Skip button) shares the row.
      Expanded(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(title, style: TextStyle(fontSize: s.f(16), fontWeight: FontWeight.w800, color: kInk), overflow: TextOverflow.ellipsis),
          if (subtitle != null) Text(subtitle!, style: TextStyle(fontSize: s.f(12), color: kInkFaint), overflow: TextOverflow.ellipsis, maxLines: 2),
        ]),
      ),
    ]);
  }
...
// Renders a label where a trailing ' *' is shown in red.
Widget _labelWithRedAsterisk(String label, {required double fontSize, required FontWeight fontWeight, required Color color}) {
  if (label.endsWith(' *')) {
  ...
  return Text(label, style: TextStyle(fontSize: fontSize, fontWeight: fontWeight, color: color));
}
...
class _AppTextField extends StatefulWidget {
  ...
  const _AppTextField({required this.controller, required this.label, required this.hint, this.isNumber = false, this.maxLines = 1, this.maxLength, this.showCounter = true, this.isInvalid = false, this.inputFormatters});
...
class _AppTextFieldState extends State<_AppTextField> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        _labelWithRedAsterisk(widget.label, fontSize: s.f(12.5), fontWeight: FontWeight.w700, color: kInkLight),
        if (widget.isInvalid) ...[
          SizedBox(width: s.s(6)),
...
class _DropdownField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        _labelWithRedAsterisk(label, fontSize: s.f(12.5), fontWeight: FontWeight.w700, color: kInkLight),
        if (infoText != null) ...[
...
class _CnicFieldState extends State<_CnicField> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      RichText(text: TextSpan(text: 'CNIC', style: TextStyle(fontSize: s.f(12.5), fontWeight: FontWeight.w700, color: kInkLight), children: const [TextSpan(text: ' *', style: TextStyle(color: Colors.red))])),
      SizedBox(height: s.s(6)),
      Stack(children: [
        TextFormField(
...
class _PhoneFieldState extends State<_PhoneField> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        RichText(text: const TextSpan(text: 'Phone', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: kInkLight), children: [TextSpan(text: ' *', style: TextStyle(color: Colors.red))])),
        if (widget.isInvalid) ...[const SizedBox(width: 6), const Icon(Icons.error_rounded, color: kRose, size: 14)],
      ]),
      const SizedBox(height: 3),
      Text('This number will be used for future verification',
        style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w500, color: kInkFaint)),
      const SizedBox(height: 6),
      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        GestureDetector(
          onTap: () => _openPicker(context),
...
class _AgeField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        RichText(text: const TextSpan(text: 'Age', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: kInkLight), children: [TextSpan(text: ' *', style: TextStyle(color: Colors.red))])),
...
class _HeightDropdownsState extends State<_HeightDropdowns> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        widget.required
          ? RichText(text: TextSpan(text: widget.label, style: TextStyle(fontSize: s.f(12.5), fontWeight: FontWeight.w700, color: labelColor), children: const [TextSpan(text: ' *', style: TextStyle(color: Colors.red))]))
          : Text(widget.label, style: TextStyle(fontSize: s.f(12.5), fontWeight: FontWeight.w700, color: labelColor)),
...
// ── Coupon Code Input Box with realtime validation ────────────────────────────
// ── Submitter type radio selector ────────────────────────────────────────────
class _SubmitterTypeSelector extends StatefulWidget {
  final ValueChanged<String?> onChanged;
  final String? initialValue;
  const _SubmitterTypeSelector({required this.onChanged, this.initialValue});
  @override State<_SubmitterTypeSelector> createState() => _SubmitterTypeSelectorState();
}

class _SubmitterTypeSelectorState extends State<_SubmitterTypeSelector> {
  String? _selected;

  @override
  void initState() {
    super.initState();
    // Restore selection if coming back to this step
  ...
  static const _options = [
    ('self',     'I am submitting this profile for myself to find a suitable marriage proposal.'),
    ('guardian', 'I am a parent/guardian submitting this profile to find a suitable marriage proposal for my son/daughter.'),
  ];

  @override
  Widget build(BuildContext context) {
    final s = _S.of(context);
    return Column(children: _options.map((opt) {
      final selected = _selected == opt.$1;
      return GestureDetector(
        onTap: () { setState(() => _selected = opt.$1); widget.onChanged(opt.$1); },
        child: Container(
          margin: EdgeInsets.only(bottom: s.s(10)),
          padding: EdgeInsets.all(s.s(14)),
          decoration: BoxDecoration(
            color: selected ? kPurpleLight : kCardBg,
            border: Border.all(color: selected ? kPurple : kBorder, width: 1.5),
            borderRadius: BorderRadius.circular(s.s(12)),
          ),
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              width: s.d(18), height: s.d(18),
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                border: Border.all(color: selected ? kPurple : kInkLight, width: 2),
                color: selected ? kPurple : kCardBg,
              ),
              child: selected ? Center(child: Container(width: s.d(8), height: s.d(8), decoration: const BoxDecoration(shape: BoxShape.circle, color: Colors.white))) : null,
            ),
            SizedBox(width: s.s(12)),
            Expanded(child: Text(opt.$2, style: TextStyle(fontSize: s.f(13), color: selected ? kPurple : kInk, fontWeight: selected ? FontWeight.w600 : FontWeight.w400, height: 1.5))),
          ]),
        ),
      );
    }).toList());
...
class _CodesDropdown extends StatefulWidget {
  final TextEditingController? couponCtrl;
  final TextEditingController? affiliateCtrl;
  const _CodesDropdown({this.couponCtrl, this.affiliateCtrl});
  @override State<_CodesDropdown> createState() => _CodesDropdownState();
}

class _CodesDropdownState extends State<_CodesDropdown> {
  bool _open = false;
    ...
    return Container(
      ...
      child: Column(children: [
        GestureDetector(
          ...
          child: Container(
            padding: EdgeInsets.symmetric(horizontal: s.s(16), vertical: s.s(12)),
            decoration: BoxDecoration(color: kPurpleLight, borderRadius: BorderRadius.circular(s.s(12))),
            child: Row(children: [
              Icon(Icons.local_offer_rounded, size: s.d(15), color: kPurple),
              SizedBox(width: s.s(8)),
              Expanded(child: Text('Have a Coupon or Referral Code?', style: TextStyle(fontSize: s.f(13), fontWeight: FontWeight.w700, color: kPurple))),
              Icon(_open ? Icons.keyboard_arrow_up_rounded : Icons.keyboard_arrow_down_rounded, color: kInkLight, size: s.d(20)),
            ]),
          ),
        ),
        if (_open) Padding(
          ...
          child: Column(children: [
            SizedBox(height: s.s(12)),
            if (widget.couponCtrl != null) _CouponInputBox(ctrl: widget.couponCtrl!),
            if (widget.couponCtrl != null && widget.affiliateCtrl != null) SizedBox(height: s.s(12)),
            if (widget.affiliateCtrl != null) _AffiliateInputBox(ctrl: widget.affiliateCtrl!),
          ]),
        ),
      ]),
    );
...
class _CouponInputBox extends StatefulWidget {
  final TextEditingController ctrl;
  const _CouponInputBox({required this.ctrl});
  @override
  State<_CouponInputBox> createState() => _CouponInputBoxState();
}

class _CouponInputBoxState extends State<_CouponInputBox> {
  bool _loading = false;
  bool? _valid; // null=untouched, true=valid, false=invalid
  String? _message;

  Future<void> _apply() async {
    final code = widget.ctrl.text.trim();
    if (code.isEmpty) return;
    setState(() { _loading = true; _valid = null; _message = null; });
    try {
      ...
      if (res == null || res['active'] != true || expired) {
        setState(() {
          _loading = false;
          _valid = false;
          _message = expired ? 'This coupon has expired' : 'Invalid or inactive coupon code';
        });
        return;
      }
      ...
      final trialDays = (res['trial_days'] as num?)?.toInt();
      String msg;
      if (type == 'free_trial' && trialDays != null) {
        msg = 'Free trial — $trialDays days free access!';
      } else if (type == 'free_days' && freeDays != null) {
        msg = '+$freeDays bonus days will be added';
      } else {
        msg = '$pct% discount will apply';
      }
      setState(() { _loading = false; _valid = true; _message = msg; });
    } catch (_) {
      setState(() { _loading = false; _valid = false; _message = 'Could not verify coupon'; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: kAmberLight,
        borderRadius: BorderRadius.circular(s.s(12)),
        border: Border.all(color: _valid == true ? kAmber : _valid == false ? kRose.withOpacity(0.5) : kAmber.withOpacity(0.4)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
        ...
        SizedBox(height: s.s(4)),
        Text('Get a discount or bonus days on your subscription.', style: TextStyle(fontSize: s.f(11), color: kInkLight)),
        SizedBox(height: s.s(10)),
        Row(children: [
          Expanded(
            child: TextField(
              ...
              onChanged: (_) => setState(() { _valid = null; _message = null; }),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: _loading
                  ? SizedBox(width: s.d(16), height: s.d(16), child: const CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                  : _valid == true
                      ? Icon(Icons.check_rounded, color: Colors.white, size: s.d(16))
                      : Text('Apply', style: TextStyle(fontSize: s.f(12), fontWeight: FontWeight.w700, color: Colors.white)),
            ),
          ),
        ]),
        if (_message != null) ...[
          SizedBox(height: s.s(6)),
          Row(children: [
            Icon(_valid == true ? Icons.check_circle_rounded : Icons.error_rounded,
              size: s.d(13), color: _valid == true ? kGreen : kRed),
            SizedBox(width: s.s(4)),
            Text(_message!, style: TextStyle(fontSize: s.f(11), fontWeight: FontWeight.w600,
              color: _valid == true ? kGreen : kRed)),
          ]),
        ],
      ]),
    );
...
// ── Affiliate Code Input Box with realtime validation ─────────────────────────
class _AffiliateInputBox extends StatefulWidget {
  final TextEditingController ctrl;
  const _AffiliateInputBox({required this.ctrl});
  @override
  State<_AffiliateInputBox> createState() => _AffiliateInputBoxState();
}

class _AffiliateInputBoxState extends State<_AffiliateInputBox> {
  bool _loading = false;
  bool? _valid;
  String? _message;
  ...
  Future<void> _apply() async {
    ...
    setState(() { _loading = true; _valid = null; _message = null; });
    try {
      // Can't select from `affiliates` directly — that table has no
      // public SELECT policy (it holds phone numbers, password hashes,
      // commission totals), so an anon client always gets an empty
      // result here regardless of whether the code is real. Goes through
      // the same SECURITY DEFINER RPC the website uses instead, which
      // only ever returns the code + name for a real, non-deleted
      // affiliate.
      final res = await SupabaseService.instance.client
          .rpc('validate_affiliate_code', params: {'p_code': code})
          .maybeSingle();
      if (res == null) {
        setState(() { _loading = false; _valid = false; _message = 'Invalid referral code'; });
        return;
      }
      setState(() { _loading = false; _valid = true; _message = 'Referral code applied'; });
    } catch (_) {
      setState(() { _loading = false; _valid = false; _message = 'Could not verify referral code'; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: kPurpleLight,
        borderRadius: BorderRadius.circular(s.s(12)),
        border: Border.all(color: _valid == true ? kPurple : _valid == false ? kRose.withOpacity(0.5) : kPurpleMid),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
