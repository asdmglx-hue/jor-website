        ...
        <div style={{ border: `2px dashed ${file ? '#534AB7' : errorField === fieldKey ? '#DC2626' : '#E8E6F5'}`, borderRadius: 12, background: file ? '#EEEDFE' : errorField === fieldKey ? '#FEF2F2' : '#FAFAFA', overflow: 'hidden', height: 140, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          {preview
            ...
            : <div style={{ textAlign: 'center', color: '#68629C' }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 600, wordBreak: 'break-word', lineHeight: 1.3, padding: '0 4px' }}>Tap to upload {label}</div>
                <div style={{ fontSize: 10, marginTop: 2 }}>JPG, PNG supported</div>
              </div>
          }
          {file && (
...
export default function ProposalFormClient() {
  ...
  const validateStep = (): { msg: string; field: string } => {
    ...
    // Step 4 (Verification)
    if (step === 4) {
      // Pair enforcement: CNIC is always front+back — never accept one side alone,
      // even when verification is not compulsory.
      if ((cnicFront && !cnicBack) || (!cnicFront && cnicBack)) {
        setVerifError(true);
        return fail('Please upload both sides of the Candidate CNIC (front and back)', 'cnic_front');
      }
      if ((guardianCnicFront && !guardianCnicBack) || (!guardianCnicFront && guardianCnicBack)) {
        setVerifError(true);
        return fail('Please upload both sides of the Guardian CNIC (front and back)', 'cnic_front');
      }
      // Compulsory check — only when admin has required verification
      if (requireVerifStep) {
  ...
  const skip = (target: 4 | 5 = 4) => {
    // Even when skipping, enforce CNIC pair rule: both sides or neither.
    // A user who uploaded only one side of a CNIC cannot skip past it —
    // they must either complete the pair or remove the single side first.
    if (target === 5) {
      if ((cnicFront && !cnicBack) || (!cnicFront && cnicBack)) {
        setSkipError('Please upload both sides of the Candidate CNIC (front and back), or remove the single side before skipping.');
        return;
      }
      if ((guardianCnicFront && !guardianCnicBack) || (!guardianCnicFront && guardianCnicBack)) {
        setSkipError('Please upload both sides of the Parent/Guardian CNIC (front and back), or remove the single side before skipping.');
        return;
      }
    }
    setSkipError('');
  ...
  const handleSubmit = async () => {
    ...
    const { success, error: apiErr } = await submitProposal({
      ...
      auth_phone: authPhone || undefined,
  ...
  if (submitted) return (
    <div style={{ maxWidth: 500, margin: '80px auto 0', padding: '0 20px', textAlign: 'center' }}>
      ...
      <p style={{ color: '#6B6893', marginBottom: 24, lineHeight: 1.6 }}>
        Thank you for submitting your profile. Please complete the payment and verification to activate your account.
      </p>
      <Link href="/my-profile" style={{ display: 'inline-block', padding: '13px 32px', borderRadius: 12, background: '#534AB7', color: '#fff', fontWeight: 800, fontSize: 15, textDecoration: 'none' }}>
        Continue to Profile
      </Link>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '32px 20px' }}>
      ...
      {showLogoutModal && (
        <div onClick={() => setShowLogoutModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 20, padding: 24, maxWidth: 340, width: '100%' }}>
      ...
      {/* Step indicator — render immediately with default (no verification step).
           Updates once settings load. This avoids the visible delay while keeping
           the Verification tab from permanently showing if settings disable it. */}
        ...
        {steps.map((s, i) => {
          ...
          return (
            <div key={i} style={{ flex: 1, textAlign: 'center', cursor: reachable ? 'pointer' : 'default' }}
              onClick={async () => {
                if (!reachable) return;
                const targetStep = (noVerifSections && i === 2 ? 5 : realStep) as 1 | 2 | 3 | 4 | 5;
                if (targetStep > step) {
                  const err = await validateStepAsync();
              ...
              <div style={{ height: 4, borderRadius: 4, background: active ? '#534AB7' : '#E8E6F5', marginBottom: 6 }} />
              <span className="step-label-desktop" style={{ fontSize: 10, fontWeight: 700, color: active ? '#534AB7' : '#68629C', textDecoration: reachable ? 'underline' : 'none' }}>{s}</span>
              <span className="step-label-mobile" style={{ fontSize: 10, fontWeight: 700, color: active ? '#534AB7' : '#68629C', textDecoration: reachable ? 'underline' : 'none' }}>{stepsMobile[i]}</span>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ background: '#fff', border: '1px solid #E8E6F5', borderRadius: 20, padding: '28px' }}>
        ...
        {step === 4 && settingsLoaded && !noVerifSections && (
          <div>
            ...
            <div style={{ background: (skipError || verifError) ? '#FEE2E2' : '#EEEDFE', borderRadius: 12, padding: '12px 16px', marginBottom: 20, fontSize: 13, color: (skipError || verifError) ? '#DC2626' : '#534AB7', lineHeight: 1.6 }}>
              {skipError || (requireVerifStep ? 'The following documents are required to verify your identity.' : 'Documents are required for identity verification, but you can submit them later.')}
            </div>
            ...
            {requireLatestDegree && (
              <CnicUploadBox label="Recent Education Document" fieldKey="educationDocument" errorField={errorField}
