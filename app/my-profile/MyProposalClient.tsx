  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '24px 20px' }}>
      ...
      <div style={{
        ...
        {user.proposal_number > 0 && <div className="hash-mobile" style={{ display: 'none', position: 'absolute', top: 20, right: 20, fontSize: 13, color: '#6B6893', lineHeight: 1 }}>#{user.proposal_number}</div>}
        <div className="my-account-left my-account-left-inner" style={{ display: 'flex', gap: 16, alignItems: 'flex-start', minWidth: 0, flex: 1 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, width: '100%' }}>
              <div style={{ fontSize: 20, fontWeight: 900, color: '#1A1830', whiteSpace: 'nowrap' }}>
                <span className="name-mobile">{user.name && user.name.length > 9 ? user.name.slice(0, 9) + '…' : user.name}</span>
                <span className="name-desktop">{user.name && user.name.length > 25 ? user.name.slice(0, 25) + '…' : user.name}</span>
              </div>
        ...
        <div className="my-account-right" style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
          ...
          {(() => {
            ...
            {settingsLoaded && (user.status === 'pending' || user.status === 'active') && !isRejected && user.cnic_verified !== true && (() => {
              ...
              return (
                <button className="desktop-only" onClick={() => { setVerifyModalOpen(true); trackEvent('verify_now_click', { source: 'desktop' }); }}
        ...
        if (label === 'Pending') return (
            ...
            {settingsLoaded && user.cnic_verified !== true && !isRejected && (() => {
              ...
              return (
                <button className="mobile-only" onClick={() => { setVerifyModalOpen(true); trackEvent('verify_now_click', { source: 'mobile' }); }}
                  ...
                  <span style={{ fontSize: 13, fontWeight: 700, color: '#7C3AED' }}>{label}</span>
                </button>
              );
            })()}
          ...
          {settingsLoaded && (() => {
            ...
            const ItemIcon = ({ done, rejected, num }: { done: boolean; rejected: boolean; num: number }) => {
              if (done) return (
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: '#DCFCE7', border: '2px solid #16A34A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              if (rejected) return (
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: '#FEE2E2', border: '2px solid #DC2626', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              return (
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: '#FEF3C7', border: '2px solid #D97706', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <span style={{ fontSize: 10, fontWeight: 800, color: '#92400E' }}>{num}</span>
                </div>
              );
            };
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    {available > 0 ? (
                      ...
                      <button onClick={() => { setPayProofType('new'); setShowFeaturedPayProofModal(true); }} style={{ display: 'inline-block', textAlign: 'center', padding: '9px 20px', borderRadius: 10, border: 'none', background: '#fff', color: isRunning ? '#E8620A' : '#534AB7', fontWeight: 800, fontSize: 13, cursor: 'pointer' }}>
                        Buy Credits
                      </button>
                    )}
            ...
            <div className="featured-mobile-layout" style={{ display: 'none' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {available > 0 ? (
                  ...
                  <button onClick={() => { setPayProofType('new'); setShowFeaturedPayProofModal(true); }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '9px 0', borderRadius: 10, border: 'none', background: '#fff', color: isRunning ? '#E8620A' : '#534AB7', fontWeight: 800, fontSize: 13, cursor: 'pointer', minHeight: 38 }}>
                    Buy Credits
                  </button>
                )}
