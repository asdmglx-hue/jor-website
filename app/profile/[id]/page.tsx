import { fetchProposalByNumber, heightDisplay, supabase } from '@/lib/supabase';
...
// anyone visits it, then stays cached for 10 minutes before the next
// visitor triggers a background refresh. New profiles work immediately,
// with no rebuild needed at all.
//
// Raised from 5 to 10 minutes (a deliberately moderate first step, not
// jumping straight to 15+) specifically to help with Error 1102 on this
// page — every rebuild is a chance for Cloudflare's free-plan 10ms CPU
...
export default async function ProposalDetailPage({ params }: Props) {
  ...
  const [p, badgeSetting] = await Promise.all([
    getCachedProposal(num),
    supabase.from('app_settings').select('value').eq('key', 'verification_badge_enabled').maybeSingle(),
  ]);
  if (!p) notFound();

  const badgeEnabled = badgeSetting.data?.value !== 'false';
      ...
      <div className="proposal-detail-grid">
                ...
                <div style={{ marginBottom: 4, minWidth: 0 }}>
                  <ProfileName
                    ...
                    badgeSuffix={p.is_doc_verified && badgeEnabled ? (
                      <span title="Verified" style={{ display: 'inline-block', verticalAlign: 'middle', lineHeight: 1, position: 'relative', top: '-1px' }}><svg viewBox="0 0 24 24" width="18" height="18" fill="#16A34A" style={{ display: 'inline', verticalAlign: 'middle', marginLeft: 5 }} aria-label="Verified">
        ...
        <div className="proposal-sidebar">
          <div style={{ background: '#fff', border: '1px solid #E8E6F5', borderRadius: 20, padding: '24px', boxShadow: '0 4px 20px rgba(83,74,183,0.1)' }}>
            <h3 style={{ fontSize: 16, fontWeight: 800, color: '#1A1830', marginBottom: 16 }}>Contact</h3>
