...
// Same pattern as /api/upload-cnic — direct R2 binding, no signing needed.
// Used by the affiliate registration form (CNIC front + back).
...
export async function POST(request: Request) {
  try {
    ...
    for (const [key, file] of [['front', front], ['back', back]] as const) {
      ...
      const objectPath = `affiliates/cnic_${key}_${Date.now()}.${ext}`;
