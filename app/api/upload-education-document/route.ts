...
// rules, same response shape, but a single optional file (no front/back
// pair, no slot) for the applicant's most recent education document,
// shown in the Verification step.
...
export async function POST(request: Request) {
  try {
    ...
    const objectPath = `proposals/${cnicDigits}/education_document_${Date.now()}.${ext}`;
