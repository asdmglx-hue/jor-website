...
// rules, same response shape. Kept as a separate route (rather than a
// `type` param on the original) so the object path is unambiguous:
// proposals/<cnic>/guardian_cnic_front_*.jpg vs cnic_front_*.jpg.
...
export async function POST(request: Request) {
  try {
    ...
    if (!(front instanceof File) || !(back instanceof File)) {
      return jsonResponse({ error: 'Both guardian CNIC front and back photos are required' }, 400);
    }

    const uploaded: Record<string, string> = {};
    for (const [key, file] of [['front', front], ['back', back]] as const) {
      if (!ALLOWED_TYPES.includes(file.type)) {
        return jsonResponse({ error: `Invalid file type for guardian CNIC ${key} photo` }, 400);
      }
      if (file.size > MAX_FILE_BYTES) {
        return jsonResponse({ error: `Guardian CNIC ${key} photo is too large (max 8MB)` }, 400);
      }

      const ext = file.type === 'image/png' ? 'png' : file.type === 'image/webp' ? 'webp' : 'jpg';
      const objectPath = `proposals/${cnicDigits}/guardian_cnic_${key}_${Date.now()}.${ext}`;
