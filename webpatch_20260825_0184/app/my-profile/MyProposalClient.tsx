        ...
        <div className="my-account-right" style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
          ...
          <div className="my-account-actions" style={{ display: 'flex', gap: 8, flexWrap: 'nowrap', alignItems: 'center' }}>
            ...
            {user.status === 'pending' && (() => {
              ...
              if (anyRejected) return true;           // always show when something rejected
              if (user.is_doc_verified) return false; // fully verified — hide
              if (hasPendingVerification) return false; // waiting for review — hide
              return true;                            // not verified, nothing pending — show
            })() && (
