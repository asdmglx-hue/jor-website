...
// cause of the follow-up bugs — scrolling back up after reaching the
// bottom frequently didn't register, and the motion felt laggy compared
// to native scrolling. Plain `overflow-y: auto` already scrolls on hover
...
// the box to fit exactly 6 stories before that native scrolling kicks in.
export default function StoryScrollBox({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    function measure() {
      ...
      if (rows.length <= VISIBLE_COUNT) { setMaxHeight(undefined); return; } // everything fits — no cap needed
      const sixthRow = rows[VISIBLE_COUNT - 1];
      const elTop = el!.getBoundingClientRect().top;
      const sixthBottom = sixthRow.getBoundingClientRect().bottom;
      // Bottom edge of the 6th story, measured from the box's own top —
      // exactly enough room for 6 regardless of how long any one quote
      // happens to run, plus a little breathing room to match the box's
      // own top/bottom padding.
      setMaxHeight(Math.ceil(sixthBottom - elTop) + 8);
    }
