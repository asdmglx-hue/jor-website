...
export default function Navbar({ sticky = false }: { sticky?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (s?.id && !s.id.startsWith('admin:')) {
      supabase.rpc('update_last_seen', { p_proposal_id: s.id, p_source: 'web' }).then(() => {});
    }
          ...
          {mounted && (user ? (
            <div style={{ position: 'relative' }}>
              <button onClick={() => setMenuOpen(menuOpen === 'user' ? false : 'user')} style={{
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: '#534AB7', maxWidth: 70, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {(user.name || '').split(' ')[0]}
                </span>
                ▾
              </button>
