'use client';
import { Proposal, heightDisplay, isSubscriptionActive, phoneDisplay } from '@/lib/supabase';
...
async function fetchBadgeEnabled(): Promise<boolean> {
  if (_badgeEnabled !== null) return _badgeEnabled;
  try {
...
export default function ProposalCard({ proposal: p, onNotInterested, onSavedChange, index, forceFeatured }: Props) {
  ...
  useEffect(() => {
    ...
    setIsPaused(session ? session.status === 'paused' : false);
  ...
  return (
    <>
      <Link href={`/profile/${p.proposal_number}`} draggable={false} onClick={() => trackEvent('profile_view', { proposal_number: p.proposal_number, gender: p.gender ?? '' })} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          {isActive ? (
            showPhone ? (
              <a
                ...
                <span style={{ fontSize: 12.5, fontWeight: 700, color: '#534AB7' }}>{phoneDisplay(p.contact_phone)}</span>
              </a>
            ) : (
              <div
                onClick={e => { e.preventDefault(); e.stopPropagation(); if (isPaused) { const ev = new CustomEvent('jor:paused-toast'); window.dispatchEvent(ev); return; } setShowPhone(true); }}
