...
export default function VerifyNowModal({ user, onClose, onSaved }: {
  ...
  useEffect(() => {
    supabase.from('app_settings').select('key, value').then(({ data }) => {
      ...
      // Only apply the admin "hide" flag if the section isn't already done.
      if (!candidateCnicAlreadyDone && map['require_candidate_cnic'] === 'false') setShowCandidateCnic(false);
      if (!degreeAlreadyDone        && map['require_latest_degree']  === 'false') setShowLatestDegree(false);
      if (!parentsCnicAlreadyDone   && map['require_parents_cnic']   === 'false') setShowParentsCnic(false);
    });
  }, []);
      ...
      <div style={{ background: '#fff', borderRadius: 20, maxWidth: 480, width: '100%', maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: 24, overflowY: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <UploadBox label="CNIC Back" file={cnicBack} preview={cnicBackPreview} compressing={compressingCnicBack}
              onFileSelected={async raw => {
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <UploadBox label="CNIC Back" file={guardianCnicBack} preview={guardianCnicBackPreview} compressing={compressingGuardianCnicBack}
              onFileSelected={async raw => {
